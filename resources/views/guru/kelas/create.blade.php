...
<div class="row mb-4">
    <div class="col-12">
        <div class="d-flex align-items-center">
            ...
            <div>
                <h3 class="mb-0">{{ isset($kelas) ? 'Edit Kelas' : 'Tambah Kelas Baru' }}</h3>
                <p class="text-muted small mb-0">{{ isset($kelas) ? 'Perbarui informasi kelas.' : 'Tambahkan kelas baru untuk distribute murid.' }}</p>
            </div>
        </div>
    </div>
</div>
    ...
    <div class="col-md-8">
        <div class="card border-0 shadow-sm" style="border-radius: 20px;">
            <div class="card-body p-4">
                <form method="POST" action="{{ isset($kelas) ? route('guru.kelas.update', ['kelas' => $kelas->id]) : route('guru.kelas.store') }}">
                    @csrf
                    @if(isset($kelas))
                        @method('PUT')
                    @endif
                    ...
                    <div class="d-flex justify-content-end gap-2 mt-4">
                        ...
                        <button type="submit" class="btn btn-primary rounded-pill px-4 shadow-sm">
                            <i class="bi bi-check-lg me-2"></i>{{ isset($kelas) ? 'Simpan Perubahan' : 'Simpan Kelas' }}
                        </button>
                    </div>
                </form>
            </div>
